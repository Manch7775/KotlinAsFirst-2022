...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    val res = str.map {
        ...
        if (num > 0) {
            val spaces1 = spaces / num
            var spaces2 = spaces % num
            var str1 = ""
            for (i in it) {
                if (i != ' ')
                    str1 += i
                else if (spaces2 > 0) {
                    str1 += " ".repeat(spaces1 + 2)
                    spaces2--
                } else str1 += " ".repeat(spaces1 + 1)
            }
            str1
        } else it
    }
    val f1 = File(outputName).bufferedWriter()
    f1.write(res.joinToString("\n"))
    f1.close()
}
...
fun top20Words(inputName: String): Map<String, Int> {
    ...
    var d = 20
    while (d < s.size && s[d].second == s[d - 1].second)
        d++
    return s.take(d).toMap()
}
